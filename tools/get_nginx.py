...
FILE_PATH = 'C:/Users/huamaolin/Desktop/nginx/allconfig/'

z_api = ZabbixAPI(ZABBIX_SERVER)
z_api.login('Admin', 'abcd@1234')
ips = []
for k in z_api.host.get(groupids='40', output=["host"]):
    ips.append(k['host'])

ips = ['10.11.31.151', '10.11.31.152', '10.11.31.154', '10.11.32.154', '10.11.31.157', '10.11.33.71', '10.11.31.155']
...
class SaltTool(object):
    ...
    def get_configs(self):
        result = None
        send_data = dict(tgt=self.tgt, func='cmd.shell', arg=['ls /usr/local/nginx/vhosts'])
        rs = self.salt_api(send_data)
        if rs['retcode'] == 0:
            result = [str(x) for x in rs['stdout'][self.tgt].strip().split('\n') if str(x).endswith('.conf')]
    ...
    def get_file(self, file_path, count):
        send_data = dict(tgt=self.tgt, func='cp.get_file_str', arg=['/usr/local/nginx/vhosts/%s' % file_path])
        rs = self.salt_api(send_data)
        if rs['retcode'] == 0:
